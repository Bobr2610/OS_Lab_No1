# IPC code examples for OS course

Этот базовый пример демонстрирует как:
- Создать новый процесс (`fork`)
- Заменить образ процесса (`exec`)
- Создать канал связи (`pipe`)
- Заменить файловые дескрипторы (`dup2`)
- `write` в / `read` из файловых дескрипторов
- `open` / `close` файлы
- `wait` до завершения дочернего процесса
- Бонус: чтение символических ссылок через `readlink`

Родительский процесс перенаправляет ввод пользователя в дочерний процесс через pipe, где числа суммируются и результат записывается в файл.
